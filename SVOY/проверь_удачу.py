...
def check_guess():
    ...
    try:
        ...
        if 0 <= user_input <= 10:
            ...
            count += 1
            if w == user_input:
                messagebox.showinfo("Поздравляем!", "ПОВЕЗЛО УГАДАТЬ")
                if count <= 3:
                    messagebox.showinfo("Поздравляем!", "В ТЕЧЕНИИ МИНУТЫ, ТВОЯ УДАЧА РАВНА 100%!!!")
                elif count > 3:
                    messagebox.showinfo("Поздравляем!", "СЕГОДНЯ УДАЧИ НЕТ")
...
button_check = tk.Button(
    root, text="ЖМИ СЮДА", command=check_guess, width=20, height=5
)
